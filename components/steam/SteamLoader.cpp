...
void SteamLoader::Initialize()
{
	if (IsSteamRunning(true))
	{
		...
		GetEnvironmentVariableW(L"PATH", pathBuffer, sizeof(pathBuffer));
		...
		m_hSteamClient = LoadLibraryW(steamDllPath.c_str());
...
uint32_t SteamLoader::GetSteamProcessId()
{
	...
	if (RegGetValueW(HKEY_CURRENT_USER, L"Software\\Valve\\Steam\\ActiveProcess", L"pid", RRF_RT_REG_DWORD, nullptr, &pid, &pidSize) == ERROR_SUCCESS)
	{
		return pid;
	}
...
std::wstring SteamLoader::GetSteamDllPath()
{
	...
	if (RegGetValueW(HKEY_CURRENT_USER, L"Software\\Valve\\Steam\\ActiveProcess", STEAMCLIENT_DLL, RRF_RT_REG_SZ, nullptr, pathString, &pathStringSize) == ERROR_SUCCESS)
	{
		return pathString;
	}
...
void SteamLoader::LoadGameOverlayRenderer(const std::wstring& baseDllPath)
{
	...
	LoadLibraryW(overlayDllPath.c_str());
}