...
//SteamComponent *steamComponent = nullptr;
static SteamComponent steamComponent;

void SteamComponent_Create() {
	//steamComponent = new SteamComponent();
...
void SteamComponent::Initialize()
{
	...
	{
		//MessageBoxA(NULL, "not running steam", "", 0);
		return;
	}
...
//#define PARENT_APP_ID 2620 //cod1 appid
#define PARENT_APP_ID 218
//#define PRODUCT_DISPLAY_NAME "\xE2\x9D\xA4" //it's a heart
//#define PRODUCT_DISPLAY_NAME "CoDExtended \xE2\x9C\x8C - CoD1 Modification"
//#define PRODUCT_DISPLAY_NAME "CoDExtended"
#define PRODUCT_DISPLAY_NAME "Kawaii of Duty"
...
void SteamComponent::InitializePresence()
{
	...
	extern char PRODUCT_DISPLAY_NAME_BUF[256];
	builder.PackString("name", PRODUCT_DISPLAY_NAME_BUF);
	builder.PackUint64("gameid", gameID);
	builder.PackString("installed", "1");
	builder.PackString("gamedir", "");
	builder.PackString("serverbrowsername", "");
	builder.PackEnd();
	...
	if (configAdded)
	{
		...
		GetModuleFileNameW(GetModuleHandle(nullptr), ourPath, _countof(ourPath));

		wchar_t ourDirectory[MAX_PATH];
		GetCurrentDirectoryW(sizeof(ourDirectory), ourDirectory);
		...
		steamUserInterface.Invoke<bool>("SpawnProcess", blob, 0, converter.to_bytes(ourPath).c_str(), converter.to_bytes(commandLine).c_str(), 0, converter.to_bytes(ourDirectory).c_str(), gameID, parentAppID, PRODUCT_DISPLAY_NAME_BUF, 0);
	}
#endif
	//IClientUser:: AdvertiseGame( CGameID gameID, CSteamID steamIDGameServer, uint32 unIPServer , uint16 usPortServer )
	//steamUserInterface.Invoke<void*>("AdvertiseGame", game_id, k_steamIDNonSteamGS, 2954848812, 28960);

	//steamUserInterface.Invoke<void*>("SetLimitedAccountCanInviteFriends", true);
}
...
bool SteamComponent::RunPresenceDummy()
{
	...
	wchar_t* commandLineMatch = wcsstr(GetCommandLineW(), steamChildPart);
